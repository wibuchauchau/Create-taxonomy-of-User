<?php
define( 'USER_CATEGORY_NAME', 'user_group' );
define( 'USER_CATEGORY_META_KEY', '_user_group' );

add_action( 'init', 'wibu_register_user_category_taxonomy' );

function wibu_register_user_category_taxonomy() {
	register_taxonomy(
...
add_action( 'admin_menu', 'wibu_add_user_categories_admin_page' );

function wibu_add_user_categories_admin_page() {
...
add_filter( 'submenu_file', 'wibu_set_user_category_submenu_active' );

function wibu_set_user_category_submenu_active( $submenu_file ) {
	global $parent_file;
...
add_action( 'show_user_profile', 'wibu_admin_user_profile_category_select' );
add_action( 'edit_user_profile', 'wibu_admin_user_profile_category_select' );

function wibu_admin_user_profile_category_select( $user ) {
	$taxonomy = get_taxonomy( USER_CATEGORY_NAME );
...
function wibu_custom_form_select( $name, $value, $options, $default_var ='', $html_params = array() ) {
	if( empty( $options ) ) {
...
add_action( 'personal_options_update', 'wibu_admin_save_user_categories' );
add_action( 'edit_user_profile_update', 'wibu_admin_save_user_categories' );


function wibu_admin_save_user_categories( $user_id ) {
	...
	if( ( current_user_can( 'administrator' ) && $_POST['role'] != 'author' ) ) {
		...
		wibu_update_users_categories_count( $previous_categories_ids, array() );
	} else {
		update_user_meta( $user_id, USER_CATEGORY_META_KEY, $new_categories_ids );
		wibu_update_users_categories_count( $previous_categories_ids, $new_categories_ids );
	}
}

function wibu_update_users_categories_count( $previous_terms_ids, $new_terms_ids ) {
	global $wpdb;
...
function wibu_show_authors_in_categories() {
